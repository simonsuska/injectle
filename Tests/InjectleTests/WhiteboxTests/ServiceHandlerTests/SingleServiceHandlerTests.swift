...
final class SingleServiceHandlerTests: XCTestCase {
    ...
    private var requestSH: SingleServiceHandler!
    
    private var unregisterSM: ScopeMock!
    private var unregisterSH: SingleServiceHandler!
    
    override func setUp() {
        // Setup for `testRequestService()`
        self.requestSM = ScopeMock(object: ServiceHandlerTestClass(value: 174))
        self.requestSH = SingleServiceHandler(scope: self.requestSM)
        ...
        self.unregisterSH = SingleServiceHandler(scope: self.unregisterSM)
